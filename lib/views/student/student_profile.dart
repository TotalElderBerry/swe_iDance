...
class StudentProfilePage extends StatelessWidget{
            ...
            child: Transform.translate(
              ...
              Obx(() {
                return DropdownButton(
                  value: (Get.find<StudentController>().isLivePanel.value == true)?1:2,
                  items: const [
                    DropdownMenuItem(
                      value: 1,
                  ...
                  onChanged: (value) {
                    if(value == 1){
                      Get.find<StudentController>().isLivePanel.value = true;
                    ...
                    if (value ==  2) {
                      Get.find<StudentController>().isLivePanel.value = false;
                      // Get.find<StudentController>().isPending.value = false;
  ...
  Widget liveClassTab(BuildContext context){
    ...
    Obx(() => DefaultTabController(
      initialIndex: Get.find<StudentController>().isLivePanel.value == true?0:0,
      length: 4, 
    child: Column(
        children: [
              TabBar(
              ...
              Expanded(
                child:  Obx((){
                  ...
                  print("inside obx");
                  return ListView.builder(itemCount: Get.find<StudentController>().filteredBookingClass.length ,itemBuilder: (context, index) {
                        return Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
  ...
  Widget recordedClassTab(BuildContext context){
    Get.find<StudentController>().filteredRecordedBookingClass.value = Get.find<StudentController>().getBookedRecordingClasses();
    return 
    Column(
      children: [
        ...
        child: TabBar(
                ...
                onTap: (int indx){
                  print(indx);
                  if(indx == 0){
                    Get.find<StudentController>().isRecordingPending.value = false;
                    ...
                    Get.find<StudentController>().isRecordingLiked.value = false;
                    Get.find<StudentController>().filteredRecordedBookingClass.value = Get.find<StudentController>().getPendingRecordedClasses();
                    print("length ${Get.find<StudentController>().filteredRecordedBookingClass.length}");
                  }else if(indx == 2){
                    Get.find<StudentController>().isRecordingBooked.value = false;
                    Get.find<StudentController>().isRecordingPending.value = false;
                ...
                tabs: [
                Tab(
                  icon: Text("Classes", style: Theme.of(context).textTheme.bodyMedium),
                ),
                Tab(
                  icon: Text("Pending", style: Theme.of(context).textTheme.bodyMedium),
          ...
          Expanded(
            child:  Obx((){
              ...
              return ListView.builder(itemCount: Get.find<StudentController>().filteredRecordedBookingClass.length ,itemBuilder: (context, index) {
                    return Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
